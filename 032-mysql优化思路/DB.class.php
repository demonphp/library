	...
	class DB {
		...
		*/
		public function __construct($dbinfo = array()) {
			...
			$this->host = isset($dbinfo['host']) ? $dbinfo['host'] : '127.0.0.1';
			...
			$this->dbname = isset($dbinfo['dbname']) ? $dbinfo['dbname'] : 'test';
			//连接认证
		...
		*/
		...
		public function db_getOne($sql) {
			//执行，返回结果
			$res = $this->db_query($sql);
			//返回结果,mysqli_fetch_assoc,有结果则返回一维数组,没有则返回false;
		...
		*/
		...
		public function db_getALL($sql) {
			...
			//返回结果,mysqli_fetch_assoc,有结果则返回一维数组,没有则返回空数组;
			return $lists;	
		}
